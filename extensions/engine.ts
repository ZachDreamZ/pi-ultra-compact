...
export class UltraCompactEngine {
	...
	public shouldCompact(currentTokens: number): boolean {
		// Gate 1: Explicit thresholdTokens (user-configured or auto-detected)
		if (currentTokens >= this.config.thresholdTokens) {
			return true;
		}
		
		// Gate 2: Percentage threshold — fires at configured % of context window
		const percentThreshold = Math.floor(this.contextWindow * 0.6);
		if (currentTokens >= percentThreshold) {
			return true;
		}
		
		// Gate 3: Hard token cap — fires at absolute token count regardless of context size
