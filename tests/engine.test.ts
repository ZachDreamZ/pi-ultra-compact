...
describe("UltraCompactEngine", () => {
	...
	describe("constructor", () => {
		...
		it("accepts custom thresholdTokens", () => {
	...
	describe("context window detection", () => {
		...
		it("detects llama-4-maverick", () => {
			const e = new UltraCompactEngine({ modelName: "llama-4-maverick" });
			expect(e.getContextWindow()).toBe(1000000);
		});

		it("falls back to family defaults for partial names", () => {
	...
	describe("reconfigure", () => {
		it("updates context window for new model", () => {
			const engine = new UltraCompactEngine({ modelName: "claude-sonnet" });
			expect(engine.getContextWindow()).toBe(200000);
		...
		it("updates threshold for new model", () => {
			const engine = new UltraCompactEngine({ modelName: "claude-sonnet" });
	...
	describe("calculateKeepTokens", () => {
		it("returns percentage-based amount", () => {
			const engine = new UltraCompactEngine({
				keepPercentage: 0.3,
	...
	describe("extractCriticalInfo", () => {
		...
		it("marks GOAL messages as critical", () => {
			const engine = new UltraCompactEngine();
		...
		it("marks ERROR messages as critical", () => {
			const engine = new UltraCompactEngine();
		...
		it("marks DECISION messages as critical", () => {
			const engine = new UltraCompactEngine();
		...
		it("marks tool messages as critical", () => {
			const engine = new UltraCompactEngine();
	...
	describe("generateSummary", () => {
		...
		it("includes previousSummary when provided", () => {
			const engine = new UltraCompactEngine();
		...
		it("does not crash when previousSummary is undefined", () => {
			const engine = new UltraCompactEngine();
			const msgs = [makeMsg("1", "user", "GOAL: Fix the bug")];
			expect(() => engine.generateSummary(msgs, undefined)).not.toThrow();
		});

		it("extracts Goals section", () => {
			const engine = new UltraCompactEngine();
		...
		it("extracts Errors & Solutions section", () => {
			const engine = new UltraCompactEngine();
		...
		it("returns non-zero compression ratio metrics", () => {
			const engine = new UltraCompactEngine();
			const msgs = [
				makeMsg(
					"1",
		...
		it("returns non-zero tokensBefore for substantial messages", () => {
			const engine = new UltraCompactEngine();
			const msgs = [
				makeMsg(
					"1",
	...
	describe("getModelRecommendations", () => {
		...
		it("returns recommendedThreshold and recommendedKeep", () => {
			const engine = new UltraCompactEngine({ modelName: "gpt-4o" });
			const rec = engine.getModelRecommendations();
	...
	describe("edge cases", () => {
		...
		it("filters out image blocks from content", () => {
			const engine = new UltraCompactEngine();
			const msgs = [
				makeStructuredMsg("1", "user", [
		...
		it("handles zero tokensBefore in compression ratio", () => {
			const engine = new UltraCompactEngine();
			const result = engine.generateSummary([]);
			expect(result.compressionRatio).toBe(1);
		});
	...
	describe("file operations in summaries", () => {
		it("extracts read file from natural language", () => {
			const engine = new UltraCompactEngine();
		...
		it("extracts modified file from natural language", () => {
			const engine = new UltraCompactEngine();
			const msgs = [makeMsg("1", "tool", "edit extensions/engine.ts")];
			engine.generateSummary(msgs);
			// File operations section should be present
		});

		it("includes File Operations section when files present", () => {
			const engine = new UltraCompactEngine();
			const msgs = [
